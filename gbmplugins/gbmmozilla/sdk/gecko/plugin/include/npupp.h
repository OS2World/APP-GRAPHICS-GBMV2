...
#if defined(_WINDOWS)
#define NP_PUBLIC_API(ResultType)	__declspec(dllexport) ResultType WINAPI
#else
#if defined(__OS2__)
#define NP_PUBLIC_API(ResultType)	ResultType _System
#else
#define NP_PUBLIC_API(ResultType)
#endif
#endif
...
NP_PUBLIC_API(NPError) NP_GetPluginData(NPPluginData * pPluginData);

#endif

NP_PUBLIC_API(NPError) NP_GetEntryPoints(NPPluginFuncs* pFuncs);

NP_PUBLIC_API(NPError) NP_Initialize(NPNetscapeFuncs* pFuncs);

NP_PUBLIC_API(NPError) NP_Shutdown();
