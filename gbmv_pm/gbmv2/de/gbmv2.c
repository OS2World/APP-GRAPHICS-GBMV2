...
static BOOL CheckFileExistsAndQueryOverride(HWND hwnd, const CHAR * filename)
{
  ...
  if (stat(filename, &buf) != -1)
  {
    if (MBID_YES != WinMessageBox(HWND_DESKTOP,
                                 hwnd,
                                 "Datei existiert bereits, Åberschreiben?",
                                 szAppName,
...
static CHAR *FileName(VOID) { return szFileName[0] != '\0' ? szFileName : "(unbenannt)"; }
    ...
    CHAR *szFileName)
{
    ...
    Caption(hwnd, " - zeichne %s", szFileName);
    if ( !MakeVisual(mod, bView, pbmp) )
    {
        /* Revert to previous filename */
        Caption(hwnd, " - %s", FileName());
        RegPri();
        Warning(hwnd, "Fehler beim Erzeugen des Bitmap Views");
        return FALSE;
    }
...
static BOOL SaveChanges(HWND hwnd)
{
    ...
    sprintf(sz, "Sichern der énderungen: %s", FileName());
    switch ( WinMessageBox(HWND_DESKTOP, hwnd, sz, szAppName, 0, MB_YESNOCANCEL | MB_WARNING | MB_MOVEABLE) )
    {
        case MBID_YES:
        {
            ...
            {
                Warning(hwnd, "Fehler beim speichern von %s: %s", gbmfild.fild.szFullFile, ModErrorString(mrc));
                return FALSE;
            }
...
static BOOL APIENTRY TrackPosHook(HAB hab, QMSG *pqmsg, ULONG msgf)
{
    ...
    if ( msgf == MSGF_TRACK )
    {
        ...
        Caption(hwndClientTracking, " - Pos. %ldx%ld", ptl.x, ptl.y);
    }
...
static BOOL APIENTRY TrackSizeHook(HAB hab, QMSG *pqmsg, ULONG msgf)
{
    ...
    if ( msgf == MSGF_TRACK )
    {
        ...
        Caption(hwndClientTracking, " - Pos. %ldx%ld, Grî·e %ldx%ld",
...
static VOID CopyToClipbrd(HWND hwndClient, RECTL rcl)
{
    ...
    if (mrc != MOD_ERR_OK)
    {
        Warning(hwndClient, "Fehler beim Kopieren auf die Zwischenablage: %s", ModErrorString(mrc));
    }
    else
    {
    ...
    )
{
    ...
    if ( mrc != MOD_ERR_OK )
    {
        Warning(hwndClient, "Fehler %s %s: %s", szWhat, FileName(), ModErrorString(mrc));
        return;
    }
...
MRESULT _System AboutDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
    switch ( msg )
    {
        ...
        {
            CHAR sz[80+1];
            sprintf(sz, "Version %4.2f, nutze GBM.DLL-Version %4.2f",
            ...
            switch (SHORT1FROMMP(mp1))
            {
                ...
                case DID_OK:
                {
                    ...
                    if ( gamaNew < 0.1 || gamaNew > 10.0 )
                        Warning(hwnd, "Gamma mu· zwischen 0.1 und 10.0 liegen");
                    else if ( shelfNew < 0.0 || shelfNew > 1.0 )
                        Warning(hwnd, "Shelf mu· zwischen 0.0 und 1.0 liegen");
                    else
                    {
                        map   = mapNew;
                        gama  = gamaNew;
            ...
            switch (SHORT1FROMMP(mp1))
            {
                ...
                case DID_OK:
                {
                    ...
                    if ( iKeepRedNew   < 0 || iKeepRedNew   > 8 )
                        Warning(hwnd, "Anzahl Bits R mu· zwischen 0 und 8 liegen");
                    else if ( iKeepGreenNew < 0 || iKeepGreenNew > 8 )
                        Warning(hwnd, "Anzahl Bits G mu· zwischen 0 and 8 liegen");
                    else if ( iKeepBlueNew  < 0 || iKeepBlueNew  > 8 )
                        Warning(hwnd, "Anzahl Bits B mu· zwischen 0 and 8 liegen");
                    else if ( nColsNew      < 1 || nColsNew      > 256 )
                        Warning(hwnd, "Anzahl Farben mu· zwischen 1 and 256 liegen");
                    else
                    {
                        iPal       = iPalNew;
                        iAlg       = iAlgNew;
...
/*...sResizeDlgProc:0:*/
static PSZ resizeLboxItems[] = { "Benachbarte Pixel",
                                 "Bilinear",
                                 "Bell",
                                 "BSpline",
                                 "Quadratisch",
...
MRESULT _System ResizeDlgProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
  switch ( msg )
  {
    case WM_INITDLG:
    {
      ...
      sprintf(sz, "Bildgrî·e %dx%d mit %d bpp",
    ...
    case WM_COMMAND:
      switch (SHORT1FROMMP(mp1))
      {
        case DID_WIDTH_MUL:
        {
          ...
          if ( w > 15000.0 )
          {
            Warning(hwnd, "Eine Breite von %6.2lf ist zu gro·", w);
            return (MRESULT) 0;
          }
          sprintf(sz, "%d", (int) w);
          WinSetDlgItemText(hwnd, DID_WIDTH, sz);
        ...
        case DID_HEIGHT_MUL:
        {
          ...
          if ( h > 15000.0 )
          {
            Warning(hwnd, "Eine Hîhe von %6.2lf ist zu gro·", h);
            return (MRESULT) 0;
          }
          sprintf(sz, "%d", (int) h);
          WinSetDlgItemText(hwnd, DID_HEIGHT, sz);
        ...
        case DID_OK:
        {
          ...
          {
            Warning(hwnd, "Kein Interpolationsfilter gewÑhlt.");
            return (MRESULT) 0;
          }
          if ( cxNew < 1 || cyNew < 1 )
          {
              Warning(hwnd, "Angegebene Grî·e ist zu klein.");
          }
          else if ( cxNew > 15000 || cyNew > 15000 )
          {
              Warning(hwnd, "Angegebene Grî·e ist zu gro·.");
          }
          else
          {
              WinDismissDlg(hwnd, TRUE);
...
MRESULT _System ObjectWndProc(HWND hwnd, ULONG msg, MPARAM mp1, MPARAM mp2)
{
    switch ( (int) msg )
    {
        ...
        case UM_OPEN:
        {
            ...
            LowPri();
            Caption(hwndClient, " - îffne %s", gbmfild.fild.szFullFile);
            mrc = ModCreateFromFile(gbmfild.fild.szFullFile, gbmfild.szOptions, &modNew);
            Caption(hwndClient, " - %s", FileName());
            RegPri();
            if ( mrc != MOD_ERR_OK )
            {
                Warning(hwndClient, "Fehler beim ôffnen %s: %s", gbmfild.fild.szFullFile, ModErrorString(mrc));
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
        ...
        case UM_SAVE:
        {
            ...
            LowPri();
            Caption(hwndClient, " - sichere %s", FileName());
            mrc = ModWriteToFile(&(drawingBitmap.mod), szFileName, "");
            Caption(hwndClient, "- %s", FileName());
            RegPri();
            if ( mrc != MOD_ERR_OK )
            {
                Warning(hwndClient, "Fehler beim Sichern %s: %s", FileName(), ModErrorString(mrc));
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
        ...
        case UM_SAVE_AS:
        {
            ...
            LowPri();
            Caption(hwndClient, " - sichere als %s", gbmfild.fild.szFullFile);
            mrc = ModWriteToFile(&(drawingBitmap.mod), gbmfild.fild.szFullFile, gbmfild.szOptions);
            Caption(hwndClient, " - %s", FileName());
            RegPri();
            if ( mrc != MOD_ERR_OK )
            {
                Warning(hwndClient, "Fehler beim Sichern als %s: %s", gbmfild.fild.szFullFile, ModErrorString(mrc));
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
        ...
        case UM_EXPORT_MET:
        {
            ...
            LowPri();
            Caption(hwndClient, " - erzeuge MetaFile");

            mrc = ModMakeHBITMAP(&(drawingBitmap.mod), hab, &hbm);
            if ( mrc != MOD_ERR_OK )
            {
                Warning(hwndClient, "Fehler bei Erzeugen des MetaFiles: %s", ModErrorString(mrc));
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
            ...
            LowPri();
            Caption(hwndClient, " - exportiere to %s", fild.szFullFile);
            ...
            if ( !fOk )
            {
                Warning(hwndClient, "Fehler beim Export nach %s", fild.szFullFile);
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
        ...
        case UM_PRINT:
        {
            ...
            Caption(hwndClient, " - drucke %s", FileName());
            LowPri();
            ...
            if ( mrc != MOD_ERR_OK )
            {
                RegPri();
                Caption(hwndClient, " - %s", FileName());
                Warning(hwndClient, "Fehler bei Druckvorbereitung %s: %s", FileName(), ModErrorString(mrc));
            }
            else if ( (brc = BmpPrint(hab, hbmCopy, colorFg, colorBg, FileName(), szAppName, NULL)) != BMP_ERR_OK )
            {
                ...
                Warning(hwndClient, "Fehler beim Drucken: %s", BmpErrorMessage(brc, sz));
            }
            else
            {
                RegPri();
        ...
        case UM_SNAPSHOT:
        {
            ...
            {
                Warning(hwndClient, "Fehler bei Bildschirmkopie: Kann PM HDC Resource nicht erzeugen");
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
            ...
            LowPri();
            Caption(hwndClient, " - Bildschirmkopie (unbenannt)");
            ...
            if ( mrc != MOD_ERR_OK )
            {
                Warning(hwndClient, "Fehler bei Bildschirmkopie: %s", ModErrorString(mrc));
                WinSendMsg(hwndClient, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }

            if ( !MakeVisualBg(hwndClient, &modNew, VIEW_NULL, &bmpNew, "(unbenannt)") )
            {
                ModDelete(&modNew);
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
        ...
        case UM_PASTE:
        {
            ...
            {
                WinCloseClipbrd(hab);
                Warning(hwndClient, "Fehler beim EinfÅgen: Kann nicht auf Zwischenablagedaten zugreifen");
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
            ...
            {
                WinCloseClipbrd(hab);
                Warning(hwndClient, "Fehler beim EinfÅgen: Kann cPlanes != 1 nicht verarbeiten");
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
            ...
            {
                WinCloseClipbrd(hab);
                Warning(hwnd, "Fehler beim EinfÅgen: Kann PM HDC Resource nicht erzeugen");
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
            ...
            LowPri();
            Caption(hwndClient, " - fÅge ein %s", FileName());
            ...
            if ( mrc != MOD_ERR_OK )
            {
                Warning(hwndClient, "Fehler beim EinfÅgen: Kann Farbtiefe des Bildes in der Zwischenablage nicht ermitteln");
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
            ...
            if ( !MakeVisualBg(hwndClient, &modNew, bView, &bmpNew, "(unbenannt)") )
            {
                ModDelete(&modNew);
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            ...
            if ( fGotBitmap )
            {
                ...
                fUnsavedChanges   = FALSE;
                fSelectionDefined = FALSE;
                KeepForUndo(&drawingBitmap, "EinfÅgen");
                SetNoBitmap();
            }
            else
                DiscardUndo();
        ...
        {
            HWND hwndClient = HWNDFROMMP(mp1);
            Reflect(hwndClient, ModReflectHorz, "spiegele horizontal", "horizontale Spiegelung");
            WinSendMsg(hwnd, UM_DONE, NULL, NULL);
            return (MRESULT) 0;
        }
        /*...e*/
        ...
        {
            HWND hwndClient = HWNDFROMMP(mp1);
            Reflect(hwndClient, ModReflectVert, "spiegele vertikal", "vertikale Spiegelung");
            WinSendMsg(hwnd, UM_DONE, NULL, NULL);
            return (MRESULT) 0;
        }
        /*...e*/
        ...
        {
            HWND hwndClient = HWNDFROMMP(mp1);
            Reflect(hwndClient, ModRotate90, "drehe um 90¯", "90¯ Drehung");
            WinSendMsg(hwnd, UM_DONE, NULL, NULL);
            return (MRESULT) 0;
        }
        /*...e*/
        ...
        {
            HWND hwndClient = HWNDFROMMP(mp1);
            Reflect(hwndClient, ModRotate180, "drehe um 180¯", "180¯ Drehung");
            WinSendMsg(hwnd, UM_DONE, NULL, NULL);
            return (MRESULT) 0;
        }
        /*...e*/
        ...
        {
            HWND hwndClient = HWNDFROMMP(mp1);
            Reflect(hwndClient, ModRotate270, "drehe um 270¯", "270¯ Drehung");
            WinSendMsg(hwnd, UM_DONE, NULL, NULL);
            return (MRESULT) 0;
        }
        /*...e*/
        ...
        {
            HWND hwndClient = HWNDFROMMP(mp1);
            Reflect(hwndClient, ModTranspose, "spiegele diagonal", "Diagonalspiegelung");
            WinSendMsg(hwnd, UM_DONE, NULL, NULL);
            return (MRESULT) 0;
        }
        /*...e*/
        ...
        case UM_CROP:
        {
            ...
            LowPri();
            Caption(hwndClient, " - schneide zu %s", FileName());
            mrc = ModExtractSubrectangle(&(drawingBitmap.mod),
                rclSelection.xLeft,
                rclSelection.yBottom,
                rclSelection.xRight - rclSelection.xLeft,
            ...
            if ( mrc != MOD_ERR_OK )
            {
                Warning(hwndClient, "Fehler beim Zuschneiden: %s", ModErrorString(mrc));
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
            ...
            KeepForUndo(&drawingBitmap, "Zuschneiden");
        ...
        case UM_COLOUR:
        {
            ...
            LowPri();
            Caption(hwndClient, " - Farbraumanpassung %s", FileName());
            mrc = ModColourAdjust(&(drawingBitmap.mod), map, gama, shelf, &modNew);
            Caption(hwndClient, " - %s", FileName());
            RegPri();
            if ( mrc != MOD_ERR_OK )
            {
                Warning(hwndClient, "Fehler bei Farbraumanpassung %s: %s", FileName(), ModErrorString(mrc));
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
            ...
            KeepForUndo(&drawingBitmap, "Farbraumanpassung");
        ...
        case UM_MAP:
        {
            ...
            {
                CHAR *szFast = "Sie haben eine Farbanpassung gewÑhlt deren Zielpalette alle Farben "
                               "der Bildschirmpalette enthÑlt. Deshalb ergeben die Algorithmen "
                               "Halbton oder Fehlerstreuung keine Verbesserung - Soll die Ansicht "
                               "auf direkte PM-Anzeige umgeschaltet werden ?";
            ...
            LowPri();
            Caption(hwndClient, " - Farbraumkonvertierung %s", FileName());
            mrc = ModBppMap(&(drawingBitmap.mod), iPal, iAlg, iKeepRed, iKeepGreen, iKeepBlue, nCols, &modNew);
            Caption(hwndClient, " - %s", FileName());
            RegPri();
            if ( mrc != MOD_ERR_OK )
            {
                Warning(hwndClient, "Fehler bei Farbraumkonvertierung %s: %s", FileName(), ModErrorString(mrc));
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
            ...
            KeepForUndo(&drawingBitmap, "Farbraumkonvertierung");
        ...
        case UM_REDEYE:
        {
            HWND hwndClient = HWNDFROMMP(mp1);
            LowPri();
            FixRedeyeSelection(hwndClient, "korrigiere Rote Augen", "Korrektur Rote Augen");
            RegPri();
            WinSendMsg(hwnd, UM_DONE, NULL, NULL);
            return (MRESULT) 0;
        }
        ...
        case UM_RESIZE:
        {
            ...
            LowPri();
            Caption(hwndClient, " - Ñndere Grî·e %s", FileName());
            ...
            if ( mrc != MOD_ERR_OK )
            {
                Warning(hwndClient, "Fehler bei Grî·enÑnderung %s: %s", FileName(), ModErrorString(mrc));
                WinSendMsg(hwnd, UM_DONE, NULL, NULL);
                return (MRESULT) 0;
            }
            ...
            KeepForUndo(&drawingBitmap, "Grî·enÑnderung");
        ...
        case WM_INITMENU:
        {
            ...
            switch ( SHORT1FROMMP(mp1) )
            {
                ...
                case MID_EDIT:
                {
                    ...
                    if ( fCanUndo )
                        sprintf(szUndo, "~RÅckgÑngig: %s\tStrg+Z", szWhatUndo);
                    else
                        sprintf(szUndo, "~RÅckgÑngig\tStrg+Z");
...
int main(int argc, CHAR *argv[])
{
    ...
    if ( argc >= 2 )
    {
        ...
        if (strcmp(gbmfilearg.argin, "\"\"") != 0)
        {
          ...
          {
            Warning(HWND_DESKTOP, "Fehler beim ôffnen %s: %s", szFileName, ModErrorString(mrc));
          }
          else if ( !MakeVisual(&modNew, bView, &bmpNew) )
          {
            ModDelete(&modNew);
            Warning(HWND_DESKTOP, "Fehler beim ôffnen %s: Kann Bitmap View nicht erzeugen", szFileName);
          }
          else
          {
            ModDelete(&modNew);
    ...
    do
        for ( ;; )
        {
            ...
            if ( !fBusy )
                break;
            Warning(hwndFrame, "Beenden nicht mîglich, %s arbeitet noch, bitte wiederholen Sie spÑter", szAppName);
        }
