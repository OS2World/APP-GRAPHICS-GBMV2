...
/* HN: Special implementation of used bytes order conversion macros in gbminet.h
       for OS/2 Warp 3/4 backward compatibility.
*/
#include "libraw/gbminet.h"
/*
#ifndef WIN32
#ifdef __WATCOMC__
#include <os2/machine/endian.h>
#elif (__GNUC__ < 4)
#include <io.h>
#else
...
#define FREE(a) do { if(a) { this->free(a); a = NULL;} }while(0)
...
int LibRaw::open_datastream(LibRaw_abstract_datastream *stream)
{
    ...
    try {
        ...
        if(C.profile_length)
            {
                if(C.profile) this->free(C.profile);
                C.profile = this->malloc(C.profile_length);
...
int LibRaw::unpack(void)
{
    ...
    try {
        ...
        if (libraw_internal_data.unpacker_data.meta_length)
            {
                libraw_internal_data.internal_data.meta_data =
                    (char *) this->malloc (libraw_internal_data.unpacker_data.meta_length);
        ...
        if(decoder_info.decoder_flags &  LIBRAW_DECODER_FLATFIELD)
            {
                imgdata.rawdata.raw_alloc = this->malloc(rwidth*rheight*sizeof(imgdata.rawdata.raw_image[0]));
        ...
        else if (decoder_info.decoder_flags &  LIBRAW_DECODER_4COMPONENT)
            {
                ...
                S.iheight= S.height;
                IO.shrink = 0;
                imgdata.rawdata.raw_alloc = this->calloc(rwidth*rheight,sizeof(*imgdata.rawdata.color_image));
        ...
        else if (decoder_info.decoder_flags & LIBRAW_DECODER_LEGACY)
            {
                ...
                imgdata.rawdata.raw_alloc = this->calloc(S.iwidth*S.iheight,sizeof(*imgdata.image));
    ...
    if(IO.fwidth) 
        {
            ...
            if(imgdata.image)
                    {
                        imgdata.image = (ushort (*)[4]) this->realloc(imgdata.image,fiheight*fiwidth*sizeof (*imgdata.image));
                        memset(imgdata.image,0,fiheight*fiwidth *sizeof (*imgdata.image));
                    }
                else
                    imgdata.image = (ushort (*)[4]) this->calloc (fiheight*fiwidth, sizeof (*imgdata.image));
    ...
    else
        {
                ...
                if(imgdata.image)
                    {
                        imgdata.image = (ushort (*)[4]) this->realloc (imgdata.image,S.iheight*S.iwidth 
                                                                 *sizeof (*imgdata.image));
                        memset(imgdata.image,0,S.iheight*S.iwidth *sizeof (*imgdata.image));
                    }
                else
                    imgdata.image = (ushort (*)[4]) this->calloc (S.iheight*S.iwidth, sizeof (*imgdata.image));
...
int LibRaw::raw2image(void)
{
    ...
    try {
        ...
        if(imgdata.image)
            {
                imgdata.image = (ushort (*)[4]) this->realloc (imgdata.image,S.iheight*S.iwidth *sizeof (*imgdata.image));
                memset(imgdata.image,0,S.iheight*S.iwidth *sizeof (*imgdata.image));
            }
        else
            imgdata.image = (ushort (*)[4]) this->calloc (S.iheight*S.iwidth, sizeof (*imgdata.image));
...
//    CHECK_ORDER_HIGH(LIBRAW_PROGRESS_PRE_INTERPOLATE);
	...
	if(O.camera_profile)
        ...
        if(!libraw_internal_data.output_data.histogram)
            {
                libraw_internal_data.output_data.histogram = (int (*)[LIBRAW_HISTOGRAM_SIZE]) this->malloc(sizeof(*libraw_internal_data.output_data.histogram)*4);
    ...
    if (T.tformat == LIBRAW_THUMBNAIL_BITMAP)
        {
            libraw_processed_image_t * ret =
                (libraw_processed_image_t *)gbmmem_malloc(sizeof(libraw_processed_image_t)+T.tlength);
    ...
    else if (T.tformat == LIBRAW_THUMBNAIL_JPEG)
        {
            ...
            libraw_processed_image_t * ret =
                (libraw_processed_image_t *)gbmmem_malloc(sizeof(libraw_processed_image_t)+dsize);
...

{
    ...
    libraw_processed_image_t *ret = (libraw_processed_image_t*)gbmmem_malloc(sizeof(libraw_processed_image_t)+ds);
    if(!ret)
        {
                if(errcode) *errcode= ENOMEM;
                return NULL;
        ...
        if(!libraw_internal_data.output_data.histogram)
            {
                libraw_internal_data.output_data.histogram =
                    (int (*)[LIBRAW_HISTOGRAM_SIZE]) this->malloc(sizeof(*libraw_internal_data.output_data.histogram)*4);
...
#define SWAP(a,b) { a ^= b; a ^= (b ^= a); }
    ...
    int  (*t_hist)[LIBRAW_HISTOGRAM_SIZE] =  (int (*)[LIBRAW_HISTOGRAM_SIZE]) this->calloc(sizeof(*t_hist),4);
    ...
    ushort (*t_curve) = (ushort*) this->calloc(sizeof(C.curve),1);
    ...
    if(T.thumb) this->free(T.thumb);
    T.thumb = (char*) this->calloc (S.width * S.height, P1.colors);
        ...
        if ( !ID.toffset)
        ...
        else
            {
                ...
                if ( write_thumb == &LibRaw::jpeg_thumb)
                    {
                        if(T.thumb) this->free(T.thumb);
                        T.thumb = (char *) this->malloc (T.tlength);
...
void LibRaw::exp_bef(float shift, float smooth)
{
    ...
    unsigned short *lut = (ushort*)this->malloc((TBLN+1)*sizeof(unsigned short));
